...
ConVar threadpool_affinity( "threadpool_affinity", "0", 0, "Enable setting affinity", 0, 0, 0, 0, &OnChangeThreadAffinity );
...
void Host_WriteConfiguration( const char *filename, bool bAllVars )
{
	...
	if ( !cbIsUserRequested && CommandLine()->CheckParm( "-default" ) )
		return;
...
void Host_CheckGore( void )
{
	...
	//
	if ( GetCurrentMod() && ( Q_stricmp( GetCurrentMod(), "cstrike" ) != 0 && Q_stricmp( GetCurrentMod(), "csso" ) != 0 ) )
