...
void C_OP_FadeAndKillForTracers::Operate( CParticleCollection *pParticles, float flStrength,  void *pContext ) const
{
	...
	for ( int i = 0; i < nLimit; i+= 4 )
	{
		...
		if ( IsAnyNegative( fl4KillMask ) )
		{
			...
			fltx4 fl4PreviousKillMask = CmpGeSIMD( fl4PreviousAge, *pLifeDuration );
			...
			fltx4 fl4UpdateMask = AndSIMD( CmpLtSIMD( fl4PreviousAge, fl4ParticleLifeTime ), fl4KillMask );
...
class C_OP_MovementPlaceOnGround : public CParticleOperatorInstance
{
	...
	uint32 GetWrittenAttributes( void ) const
	{
		return PARTICLE_ATTRIBUTE_XYZ_MASK | PARTICLE_ATTRIBUTE_PREV_XYZ_MASK | PARTICLE_ATTRIBUTE_LIFE_DURATION_MASK | PARTICLE_ATTRIBUTE_HITBOX_RELATIVE_XYZ_MASK;
	}
