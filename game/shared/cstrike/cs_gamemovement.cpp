...
ConVar sv_staminajumpcost( "sv_staminajumpcost", ".080", FCVAR_REPLICATED, "Stamina penalty for jumping", true, 0.0, false, 0.0 );
ConVar sv_staminalandcost( "sv_staminalandcost", ".050", FCVAR_REPLICATED, "Stamina penalty for landing", true, 0.0, false, 0.0 );
ConVar sv_staminarecoveryrate( "sv_staminarecoveryrate", "60", FCVAR_REPLICATED, "Rate at which stamina recovers (units/sec)", true, 0.0, false, 0.0 );
ConVar sv_staminamax( "sv_staminamax", "80", FCVAR_REPLICATED, "Maximum stamina penalty", true, 0.0, true, 100.0 );
...
ConVar sv_jump_impulse( "sv_jump_impulse", "301.993377", FCVAR_REPLICATED, "Initial upward velocity for player jumps; sqrt(2*gravity*height).", true, 0.0f, false, 0.0f );
...
void CCSGameMovement::FinishUnDuck( void )
{
	...
	player->m_Local.m_flDucktime = 0; // legacy
